...
def read_public_page(
    route: str, project_root: Path, base_url: str | None = None
) -> str:
    if base_url:
...
def validate_timeline_page(errors: list[str], project_root: Path, html: str) -> None:
    ...
    for element_id, expected in counts.items():
        ...
        if re.search(zero_pattern, html):
            errors.append(
                f"timeline page renders zero for {element_id} even though data count is {expected}"
            )
        expected_pattern = rf'id="{re.escape(element_id)}"\s*>\s*{expected}\s*<'
        if not re.search(expected_pattern, html):
            errors.append(
                f"timeline page does not expose expected {element_id} count {expected}"
            )
    first_event = (data.get("events") or [{}])[0]
    if first_event.get("title") and first_event["title"] not in html:
        errors.append(
            f"timeline page does not expose first event title {first_event['title']!r}"
        )
...
def validate_pathogen_page(errors: list[str], project_root: Path, html: str) -> None:
    ...
    for text in required_text:
        if text and text not in html:
            errors.append(
                f"pathogen page does not expose expected static text: {text[:80]!r}"
            )
    if (
        '<h3 id="origin-label"></h3>' in html
        or '<p class="panel-copy" id="origin-narrative"></p>' in html
    ):
...
def validate_maritime_page(errors: list[str], project_root: Path, html: str) -> None:
    ...
    module_list_match = re.search(
        r'<div id="module-list">(.*?)</section>', html, flags=re.S
    )
    ...
    if module_list_html.count('class="module-card"') < len(modules):
        errors.append(
            "maritime page does not expose one readable module card per module"
        )
    for module in (modules[0], modules[-1]):
        for key in ("title", "uncertainty_note"):
            ...
            if value and value not in module_list_html:
                errors.append(
                    f"maritime module list does not expose {module['id']}.{key}"
                )


def validate_revolutionary_war_page(
    errors: list[str], project_root: Path, html: str
) -> None:
    for required in (
        ...
        if required not in html:
            errors.append(
                f"revolutionary war atlas does not expose expected content: {required!r}"
            )
    if "unpkg.com/leaflet" in html or "impact rating" in html.lower():
        errors.append(
            "revolutionary war atlas restored a removed dependency or unsupported encoding"
        )


def validate_viking_page(errors: list[str], project_root: Path, html: str) -> None:
    for required in (
        "Ribe",
        "Birka",
        "Varnhem",
        ...
        if required not in html:
            errors.append(f"viking atlas is missing {required!r}")


def validate_histsearch_page(errors: list[str], project_root: Path, html: str) -> None:
    for required in (
        "maritime-quarantine",
        "revolutionary-smallpox",
        "viking-paleodemography",
        "Review status",
        "Search strategy and review trail",
    ):
        if required not in html:
            errors.append(f"Histsearch is missing {required!r}")
    if "localhost" in html or "127.0.0.1" in html:
        errors.append("Histsearch requires an unavailable local backend")


def validate_built_pages(
    project_root: Path = PROJECT_ROOT, base_url: str | None = None
) -> list[str]:
    errors: list[str] = []
    routes = {
        ...
        "atlases/viking": validate_viking_page,
        "tools/histsearch": validate_histsearch_page,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Smoke-test built public learning-tool pages for placeholders and hydration failures."
    )
    parser.add_argument(
        "--base-url",
        help="Optional local HTTP base URL to fetch, for example http://127.0.0.1:8768/",
    )
    parser.add_argument(
